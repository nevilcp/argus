...
===========================
Backtrader Strategy integration for ARGUS.
Coordinates agents, enforces PiT limits, and triggers portfolio rebalancing.
"""
...
class ARGUSBacktestStrategy(bt.Strategy):
    ...
    def next(self):
        self.bar_count += 1
        # Trade on the first day, and then every rebalance_days
        if self.bar_count != 1 and (self.bar_count - 1) % self.p.rebalance_days != 0:
            return

        # self.data.datetime.date(0) gets the current bar's date
        ...
        # Iterate over all data feeds in cerebro
        for data_feed in self.datas:
            ...
            try:
                ...
                fund = self.agents["fundamental"].analyze(
                    ticker,
                    backtest_mode=True,
                    session_seed=int(sim_date.strftime("%Y%m%d"))
                )
                sent = self.agents["sentiment"].analyze(ticker)

                # Fetch SPY for risk evaluation Beta if needed
                try:
                    spy_close = self.pit_enforcer.get_close_series("SPY")
                except Exception:
                    spy_close = close_series  # fallback to self if spy unavailable
    ...
    def _simulate_session_state(self, ticker: str, close: pd.Series) -> dict:
        """
        Approximates a 5-min session state using the daily close series.
        """
        if close.empty:
            return {"recent_prices": [], "volume": []}
            
        # Mock high-frequency compression using the last 20 daily closes as a proxy for test
        recent = close.tail(20).tolist()
        return {
            "recent_prices": recent,
            "volume": [1000] * len(recent)
        }

    def _execute_allocation(self, allocation: PortfolioAllocation):
        # We assume universe matches data feeds
        ...
        for data_feed in self.datas:
            ...
            target_pct = alloc_dict.get(ticker, 0.0)
...
def run_backtest(
    ...
    # Need SPY for Beta calculations
    ...
    for ticker in universe_with_spy:
        try:
            ...
            if not df.empty:
                # bt.feeds.PandasData requires exact column matching; yfinance v0.2.40+ returns multiindex
