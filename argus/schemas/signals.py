...
def missing_session_state_keys(state: dict) -> list[str]:
    ...
    return sorted(k for k in _NUMERIC_SESSION_STATE_KEYS if not math.isfinite(float(state[k])))
...
class PositionAllocation(BaseModel):
    ...
    advisor_note: str | None = Field(
        ...
        max_length=600,
        description=(
            "Professional multi-sentence advisory rationale generated by the "
            "Portfolio Manager (≤600 chars)"
        ),
    )
...
class PortfolioAllocation(BaseModel):
    ...
    user_investable_capital: float = Field(
        ...
        gt=0.0,
        description=(
            "Total wealth this allocation was computed against (USD) - "
            "the capital base for allocation_pct and allocation_usd, not wealth "
            "pre-scaled by invest_pct"
        ),
    )
    ...
    @classmethod
    def tickers_are_unique(cls, portfolio: list[PositionAllocation]) -> list[PositionAllocation]:
...
class ARGUSDecision(BaseModel):
    ...
    session_timestamp: datetime = Field(
        ..., description="Naive local timestamp of the decision cycle"
    )
