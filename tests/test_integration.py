...
=========================
Comprehensive integration tests for ARGUS v2.
...
from argus.agents.portfolio import PortfolioManagerAgent, half_kelly_weight
...
class TestEndToEnd:
    ...
    def test_statistical_agents_pipeline(self):
        ...
        price_history = {"AAPL": pd.Series([150.0, 151.0, 149.0, 155.0, 160.0], index=pd.date_range("2023-01-01", periods=5))}
    ...
    async def test_full_graph_smoke(self, mock_sent, mock_fund):
        ...
        def fund_side_effect(ticker, backtest_mode=False, session_seed=None):
            return FundamentalSignal(
                ticker=ticker, signal=Signal.BULLISH, conviction=0.8,
                moat_score=8.0, reasoning="Test", data_as_of_date=date.today(),
        ...
        def sent_side_effect(ticker, *args, **kwargs):
            return SentimentSignal(
                ticker=ticker, signal=Signal.BULLISH, conviction=0.7,
                finbert_net_score=0.5, pct_positive=0.6, pct_negative=0.1,
                news_volume_7d=10, social_mention_surge=False, upcoming_catalyst=False,
                sentiment_decay_risk="LOW", reasoning="Test", timestamp=datetime.now()
            )
    ...
    def test_schema_round_trip(self):
        tech = TechnicalSignal(
            ticker="AAPL", signal=Signal.BULLISH, conviction=0.8, net_score=0.7,
            rsi_14=60, macd_histogram=0.5, bb_percent_b=0.8, atr_pct=0.02, adx_14=25,
            vwap_distance=0.01, volume_ratio=1.2, momentum_30m=0.001, momentum_1d=0.01,
            timestamp=datetime.now()
        )
        ...
        macro = MacroContext(
            macro_regime=Regime.EXPANSION, interest_rate_trend="STABLE",
            yield_curve_shape=YieldCurve.NORMAL, vix_level=15.0, vix_regime=VixRegime.LOW,
            vix_percentile=30.0, inflation_trajectory="STABLE", sector_rotation_signal=SectorSignal.GROWTH_FAVORED,
            agent_multipliers={"fundamental": 1.0, "technical": 1.0, "sentiment": 1.0},
            regime_confidence=0.8, timestamp=datetime.now()
        )
    ...
    def test_half_kelly_formula(self):
        weight = half_kelly_weight(win_probability=0.6, avg_win_pct=0.08, avg_loss_pct=0.04, max_position=0.15)
