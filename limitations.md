...
- **NewsAPI Daily Request Budget and Article Delay**: The free Developer tier caps requests at 100/day. `fetch_news` is called once per ticker, so the 20-ticker default universe spends 20 of 100 requests per sweep — a 5-run/day ceiling, tighter than any Groq quota. `fetchers.fetch_news` enforces this budget locally and returns `None` (distinguishable from an empty list) once exhausted. Separately, NewsAPI's free tier delays article availability by roughly 24 hours, so the sentiment agent's news inputs are never same-day by construction.
- **yfinance and pytrends Are Unofficial Clients, Not Documented APIs**: Both drive the private JSON endpoints behind the public Yahoo Finance and Google Trends websites rather than a published, versioned API — there is no rate-limit contract, no `Retry-After` guarantee, and no SLA. yfinance's own documentation scopes it to "research and educational purposes," and neither access pattern is permitted by the respective terms of service. Acceptable for an academic project; not a production data source.
