...
- **`image.yml`** — builds `Dockerfile.api` and pushes it to GHCR as both `ghcr.io/<owner>/argus:latest` and an immutable `:sha-<commit>` tag, on every push to `main` plus a weekly rebuild for base-image security patches. The image itself is stamped with its own tag at build time (`ARGUS_IMAGE_TAG`), which every decision it produces carries in `decisions.jsonl` for provenance.
- **`collector.yml`** — runs `argus-collect` (the `scripts/collect_session.py` entry point) every 2 hours during US market hours against the pinned `:sha-<commit>` image — 5 ticks/weekday, matching NewsAPI's 5-run/day free-tier ceiling below.
- **`reconcile.yml`** — runs `argus-reconcile` once daily, after market close, against the same pinned image.

To enable: add `GROQ_API_KEY`, `FRED_API_KEY`, and `NEWSAPI_KEY` as repository secrets (**Settings → Secrets and variables → Actions**), then push to `main` so `image.yml` publishes the first image. Of the three, only `GROQ_API_KEY` is required — `collector.yml` runs a preflight step that fails the job immediately if it's absent, since the fundamental, sentiment, and portfolio agents all need it and every decision comes out null without it. `FRED_API_KEY` and `NEWSAPI_KEY` are optional: their absence only degrades the macro agent and sentiment's catalyst detection respectively, so the preflight warns rather than fails. `reconcile.yml` needs none of the three — it only reads live prices (keyless `yfinance`) and stores outcomes via a local embedding model — so nothing is plumbed into its container. Both scheduled workflows write their state — `chroma_db/`, `decisions.jsonl`, `status.json` — to an orphan `argus-data` branch, force-pushed as a single commit each run so the branch never grows unbounded. `collector.yml` and `reconcile.yml` share a concurrency group so they never race each other on that branch.

**Pinning and rollback:** `image.yml` publishes the commit tag it just built to `image-tag.txt` on a second orphan branch, `argus-deploy`, which `collector.yml` and `reconcile.yml` read at the start of each run to decide which tag to pull. To roll a scheduled job back to a known-good build with no rebuild, edit `image-tag.txt` on `argus-deploy` to a prior `sha-<commit>` and push — the next scheduled tick picks it up. The next successful `image.yml` run overwrites the pin back to its own build, so a rollback is meant to be temporary, for the duration of diagnosing a regression.
